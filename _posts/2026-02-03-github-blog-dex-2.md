...
description: Github Pages에서 Jekyll Chirpy 테마를 적용해서 생성한 정적 블로그를 삼성 덱스 환경에서 깃허브 웹페이지만으로 다뤄본 체험담을 담았습니다. 웹페이지가 정상적으로 작동하게 하는 설정, 글 올리는 방법, 검색엔진에 노출시키기 위한 sitemap.xml, feed.xml을 생성하는 방법을 다뤘습니다.
---
...
description은 홈 화면에서 블로그를 읽는 사람에게 글의 내용으로 노출되면서 검색 엔진에게 그 글의 성격을 간략히 설명하는 역할을 한다고 한다.

그밖에 다양한 내용들을 프론트 메터에 넣어볼 수가 있다. 특이한 항목을 넣는다고 해서 딱히 어떤 효과가 발휘되는 건 아니지만, 글이 등록되는데 방해가 되는 건 아니라고 한다.
...
나는 seo(검색 엔진 최적화)에 대해서는 완전히 문외한이다. 기존에 글을 써 왔던 블로그조차 태그 같은 것조차 하나도 안 달고 글만 써왔다. 다만 깃허브 페이지는 아무런 seo작업을 하지 않는다면 아무도 모르는 4차원의 무인도가 될 가능성이 다분하다. 따라서 포탈 사이트에서 제공하는 블로그와 달리 검색 엔진에게 출생신고를 하는 게 중요하다.
